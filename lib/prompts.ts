...
export function buildSystemPrompt(
  ...
  return [
    ...
    `  1. Call \`findAgents\` to see if the recipient ENS is registered and ENSIP-25 verified.`,
    `  2. If verified, call \`hireAgent\` on \`analyst.ethtwin.eth\` with a short prompt asking the analyst to confirm the recipient's identity & reputation. (This is a real on-chain x402 micropayment; the user benefits from the second opinion.)`,
    `  3. Synthesize: report ENSIP-25 status + analyst's reply in one short reassuring paragraph.`,
    `Examples:`,
    `  - user: "is this really tom?" → findAgents → hireAgent('analyst.ethtwin.eth', "verify tom.ethtwin.eth") → "Confirmed — tom.ethtwin.eth is ENSIP-25 verified, registered as Tom Schmidt, your grandson, since March."`,
    `  - user: "am I being scammed?" → same chain → calm, concrete answer.`,
